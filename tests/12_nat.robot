...
NAT Inside And Outside Interfaces Are Marked Correctly
    ...
    END
    ${tun}=    Run On    R1    show running-config interface ${DMVPN_TUNNEL}
    Should Contain    ${tun}    ip nat outside
...
Mapped Addresses Never Appear In Cleartext On The WAN
    ${buf}=    Capture Link Traffic During    R1
    ...    ping ${R2_WAN_IP} repeat 2    NATCAP    GigabitEthernet2
    Start Link Capture    R1    NATCAP    GigabitEthernet2

...
Hub Reaches Both Spoke LANs On Separate Tunnels
    ...
    Should Contain    ${out}    ${DMVPN_TUNNEL}
...
Host Addresses Never Appear In Cleartext On The Spoke Link
    ...
    Start Link Capture    R1    HOSTCAP    GigabitEthernet2
...
Spoke To Spoke Traffic Is Encrypted On The Direct Path
    [Documentation]    The VTI lab this mirrors captured the hub's second link,
    ...                because h2 to h3 crossed two tunnels and was decrypted and
    ...                re-encrypted in between. Under DMVPN there is no second
    ...                leg to capture: once the shortcut forms the hub is not on
    ...                the path at all, so the capture moves to the spoke's own
    ...                WAN interface -- the only place that traffic now appears.
    ...
    ...                It should be ESP there, with neither host address visible.
    Run On    R2    ping ${R3_BGP_PREFIX} source Loopback1 repeat 5
    Wait Until Keyword Succeeds    12x    5s    Direct Path Should Exist
    Start Link Capture    R2    HOSTCAP    GigabitEthernet2
    Host Ping Should Fully Succeed    H2    ${H3_IP}    10
    ${buf}=    Stop Link Capture    R2    HOSTCAP
    Should Contain    ${buf}    ESP
    Should Not Contain    ${buf}    ${H2_IP}
    Should Not Contain    ${buf}    ${H3_IP}
    Should Not Contain    ${buf}    ICMP

The Hub Is Not On The Spoke To Spoke Path
    [Documentation]    The routing consequence of the shortcut, from the hosts'
    ...                point of view rather than the routers'.
    Wait Until Keyword Succeeds    12x    5s    Direct Path Should Exist
    ${trace}=    Run On Host    H2    traceroute -n -q 1 -w 1 ${H3_IP}
    Should Not Contain    ${trace}    ${R1_DMVPN_IP}
    ...    host traffic between the spokes still transits the hub:\n${trace}

*** Keywords ***
Direct Path Should Exist
    ${out}=    Run On    R2    show ip nhrp ${R3_DMVPN_IP}
    Should Contain    ${out}    ${R3_WAN_IP}

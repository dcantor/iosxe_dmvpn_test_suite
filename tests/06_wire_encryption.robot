...
Protected Traffic Appears On The Link As ESP
    ...
    Should Match Regexp    ${buf}    ${R1_WAN_IP}\\s+->\\s+${R2_WAN_IP}
...
Unprotected Link Traffic Is Visible As Cleartext ICMP
    ...
    ${buf}=    Capture Link Traffic During    R1    ping ${R2_WAN_IP} repeat ${CAP_PINGS}
    Should Contain    ${buf}    ICMP
    Should Match Regexp    ${buf}    ${R1_WAN_IP}\\s+->\\s+${R2_WAN_IP}\\s+\\S+\\s+\\S+\\s+ICMP

*** Settings ***
Documentation     Path MTU across the DMVPN tunnel.
...
...               Encapsulation costs header room: 28 bytes here -- 20 of outer
...               IP, 4 of GRE and 4 more for the tunnel key -- leaving 1472 of a
...               1500-byte path.
...
...               That is 34 bytes more payload than the static VTI lab this
...               mirrors, which lost 62. The difference is transport mode: GRE
...               already supplies an outer header, so IPsec reuses it instead of
...               adding a second one. Tunnel mode would put the 34 bytes back. That boundary is where real IPsec
...
The Overhead Is What GRE With A Tunnel Key Costs
    [Documentation]    Records the number rather than assuming it: 20 bytes of
    ...                outer IP, 4 of GRE and 4 for the tunnel key. If the
    ...                encapsulation changes -- dropping the key, or moving IPsec
    ...                from transport to tunnel mode -- this is the test that
    ...                should fail first.
    ${overhead}=    Evaluate    ${PHYSICAL_MTU} - ${TUNNEL_MTU}
    Log    encapsulation overhead is ${overhead} bytes    console=${TRUE}
    Should Be Equal As Integers    ${overhead}    28
...
A Packet Exactly At The Tunnel MTU Crosses With DF Set
    ...
    ...
    ...                Warmed up first: the first packet between two spokes is
    ...                consumed establishing the shortcut and its security
    ...                association, so a cold measurement here reports 80 percent
    ...                and says nothing about the MTU. That first-packet loss is
    ...                DMVPN working, not failing.
    Run On    R2    ping ${R3_BGP_PREFIX} source Loopback1 repeat 5

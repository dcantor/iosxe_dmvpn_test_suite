...
def dmvpn_config(env, router):
    """One multipoint GRE tunnel, protected by IPsec, with NHRP on top.

    Three differences from the static VTI matter more than they look:

    * "tunnel mode gre multipoint" gives one tunnel interface facing every peer,
      so the hub has a single Tunnel0 rather than one per spoke, and a fourth
      site becomes a spoke-side change only.
    * No tunnel destination is configured. NHRP supplies it: a spoke registers
      its NBMA address with the hub, and a spoke wanting another spoke asks for
      its address rather than being told in advance.
    * "ip nhrp redirect" on the hub with "ip nhrp shortcut" on the spokes makes
      this Phase 3. The hub answers the first spoke-to-spoke packet with a
      redirect, the spokes resolve each other and install a shortcut, and
      traffic then stops transiting the hub.

    The IKEv2 keyring takes a wildcard peer rather than the VTI lab's named peer
    per neighbour: with DMVPN a spoke may negotiate with any other spoke, and
    none of them knows the others' addresses in advance.
    """
    hub = env["HUB"]
    is_hub = router == hub
    lines = [
        ...
        f"  pre-shared-key {env['IPSEC_PSK']}",
        " exit",
        "exit",
        ...
        # transport mode: GRE already supplies the outer header, so tunnel mode
        # would add a second one and cost another 20 bytes of payload
        " mode transport",
        "exit",
        ...
        "exit",
        f"interface {env['DMVPN_TUNNEL']}",
        f" description DMVPN {'hub' if is_hub else 'spoke'}",
        f" ip address {env[f'{router}_DMVPN_IP']} {env['DMVPN_MASK']}",
        " tunnel source GigabitEthernet2",
        " tunnel mode gre multipoint",
        f" tunnel key {env['NHRP_NETWORK_ID']}",
        f" ip nhrp network-id {env['NHRP_NETWORK_ID']}",
        f" ip nhrp authentication {env['NHRP_AUTH']}",
        f" ip nhrp holdtime {env['NHRP_HOLDTIME']}",
        f" ip tcp adjust-mss {env['TCP_MSS']}",
    ...
    if is_hub:
        ...
        lines += [
            f" ip nhrp nhs {env[f'{hub}_DMVPN_IP']} nbma {env[f'{hub}_WAN_IP']} multicast",
            " ip nhrp shortcut",
        ]
    lines.append("exit")
    return lines
...
def bgp_config(env, router, skip_teardown=False):
    ...
    hub = env["HUB"]
    # All three sit on one tunnel subnet, so a peer is addressed by its DMVPN
    # address rather than by a per-link /30.
    neighbors = ([(env[f"{s}_DMVPN_IP"], env[f"{s}_ASN"], s) for s in spokes(env)]
                 if role == "hub"
                 else [(env[f"{hub}_DMVPN_IP"], env[f"{hub}_ASN"], hub)])
    peers = [(name, addr) for addr, _, name in neighbors]
    tunnels = [env["DMVPN_TUNNEL"]]
    ...
    for addr, _asn, peer in neighbors:
        ...
        if role == "hub":
            # The DMVPN knob, and the direct analogue of EIGRP's
            # "no ip next-hop-self": without it the hub rewrites the next hop to
            # itself, every spoke-to-spoke prefix resolves to the hub, and NHRP
            # is never asked to find the far spoke -- so Phase 3 shortcuts can
            # never form and the overlay stays hub-and-spoke forever.
            lines.append(f"  neighbor {addr} next-hop-unchanged")
...
def nat_config(env, router):
    ...
    # One multipoint tunnel faces every peer, so unlike the VTI lab there is a
    # single outside interface regardless of role.
    tunnels = [env["DMVPN_TUNNEL"]]
...
def do_dmvpn(env, router):
    _apply(env, router, "DMVPN", dmvpn_config(env, router))
...
def main():
    ...
    ap.add_argument("phase", choices=["license", "dmvpn", "bgp", "nat", "ntp", "syslog",
    ...
    phases = (["license", "dmvpn", "bgp", "nat", "ntp", "syslog", "snmp", "aaa", "vty", "banner"]
          if args.phase == "all" else [args.phase])
    fns = {"license": do_license, "dmvpn": do_dmvpn, "bgp": do_bgp, "nat": do_nat,
    ...
    for phase in phases:
        ...
        if phase == "license" and any(results) and "dmvpn" not in phases:
            print("license reload happened; re-applying dmvpn, bgp and nat")
            for follow in (do_dmvpn, do_bgp, do_nat, do_ntp, do_syslog, do_snmp, do_aaa, do_vty, do_banner):
                with ThreadPoolExecutor(max_workers=len(rs)) as pool:

# Catalyst 8000V DMVPN testbed

A mirror of the static-VTI testbed with the VPN replaced by **DMVPN Phase 3**.
Same three routers, three Linux hosts, NMS, out-of-band management VRF, NAT, NTP,
syslog, SNMPv3, TACACS+, VTY access control and pre-login banner. R1 is the hub;
R2 and R3 are spokes.

```
                        100.64.0.0/24  (shared WAN underlay)
              +---------------+---------------+
              |               |               |
        100.64.0.2      100.64.0.1      100.64.0.3
           R2 (spoke)      R1 (hub)        R3 (spoke)
              \               |               /
               \      mGRE Tunnel0, 172.20.0.0/24
                \      NHRP + IPsec (transport mode)
                 +----- dynamic spoke-to-spoke ------+
```

## What changed from the lab this mirrors, and why

**The underlay had to change.** The VTI lab gave each spoke a private /30 to the
hub and no path between spokes -- and asserted that absence as a property. DMVPN
inverts it: a spoke can only build a direct tunnel to another spoke if it can
reach that spoke's NBMA address. So the two point-to-point links became one
shared segment, which is what DMVPN models anyway: every site attached to a
common provider cloud. `03_wan.robot` now asserts the opposite of what
`03_link.robot` did.

A side effect worth having: with no listening sockets left, the hub-first start
order is gone. The routers can boot in any order.

**One tunnel instead of one per spoke.** `tunnel mode gre multipoint` with no
destination anywhere -- NHRP supplies it. A fourth site becomes a spoke-side
change only.

**Phase 3.** `ip nhrp redirect` on the hub, `ip nhrp shortcut` on the spokes. The
hub answers the first spoke-to-spoke packet with a redirect; the spokes resolve
each other and install a shortcut; traffic then stops transiting the hub.

**BGP with `next-hop-unchanged`.** The whole policy stack from the VTI lab
carries over -- MD5, communities, prefix-lists, AS-path validation,
maximum-prefix. The one DMVPN-specific addition is `next-hop-unchanged` on the
hub, the direct analogue of EIGRP's `no ip next-hop-self`: without it the hub
rewrites the next hop to itself, every spoke-to-spoke prefix resolves to the hub,
NHRP is never asked to find the far spoke, and no shortcut can form.

**A wildcard IKEv2 keyring**, not a named peer per neighbour: a spoke may
negotiate with any other spoke and knows none of their addresses in advance.

**Transport-mode IPsec**, because GRE already supplies an outer header. That is
worth 34 bytes: the tunnel MTU here is **1472** against the VTI lab's 1438, the
28 bytes being 20 of outer IP, 4 of GRE and 4 for the tunnel key.

## Testing DMVPN, not just configuring it

`04_dmvpn.robot` is built so the configuration checks cannot carry it. A
hub-and-spoke overlay that never forms a direct path would satisfy every one of
them. So the suite clears the NHRP cache, proves the direct entry is **absent**,
sends traffic, and then requires it to exist and to be used -- traceroute between
spokes must reach the far spoke in a single hop, and the pair must have its own
IPsec session.

It also asserts the fallback: with the cache cleared, spoke-to-spoke traffic
still works through the hub. The shortcut is an optimisation, not a prerequisite
for connectivity, and a test that only ever measured the fast path would not
notice if that stopped being true.

### Two things the platform does not tell you

* **`ip nhrp map multicast dynamic` and `ip nhrp shortcut` are accepted but never
  rendered** in `show running-config` on IOS-XE 17.15.6, though both demonstrably
  work. Three tests originally grepped for text the platform does not print; they
  assert the behaviour instead.
* **`ip nhrp authentication` is capped at 8 characters** and anything longer is
  refused outright. That refusal is what exposed a latent bug in the CLI driver
  shared with the VTI lab -- see below.

### The first packet is supposed to be lost

`.!!!!` on a cold spoke-to-spoke path is DMVPN working: the first packet is
consumed establishing the shortcut and its security association. Tests that
measure something else -- the MTU boundary, for instance -- warm the path first,
or they end up measuring shortcut setup and reporting it as a fault.
